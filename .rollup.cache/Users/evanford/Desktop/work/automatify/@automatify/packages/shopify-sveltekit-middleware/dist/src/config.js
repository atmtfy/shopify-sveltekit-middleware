...
export async function loadConfig(options = null) {
    let validated = false;
    const config_file = path.join(process.cwd(), 'svelte.config.js');
    if (options == null) {
        if (!fs.existsSync('svelte.config.js')) {
            console.error('Missing svelteify.config.js — cannot init middleware');
            return;
        }
        else {
...
function integrateDefaults(config) {
    ...
    if (env) {
        ...
        console.warn('Shopify middleware must contain an \'env\' property, defined either in svelteify.config.js or in svelte.config.js.\'');
        return false;
    }
}
