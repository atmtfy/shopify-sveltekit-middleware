...
function isDev() {
    if (({ url: (typeof document === 'undefined' ? new (require('u' + 'rl').URL)('file:' + __filename).href : (document.currentScript && document.currentScript.src || new URL('middleware-81b59bf0.js', document.baseURI).href)) }) && 'env' in ({ url: (typeof document === 'undefined' ? new (require('u' + 'rl').URL)('file:' + __filename).href : (document.currentScript && document.currentScript.src || new URL('middleware-81b59bf0.js', document.baseURI).href)) }) && 'mode' in undefined && undefined.mode == 'DEVELOPMENT') {
        return true;
    }
...
function Logger(msg, level) {
    if (typeof msg != "string") {
        ...
        if (level == "error") {
            ...
            switch (level) {
                case "info":
                    console.log(bold(green('Shopify Info:')) + " " + green(msg));
                    break;
                case "warning":
                    console.log(bold(yellow('Shopify Warning:')) + " " + yellow(msg));
                    break;
            }
        }
    }
...
let ACCESS_TOKENS = {};
async function provideToken(shop, config) {
    if (shop in ACCESS_TOKENS) {
        return { shop, accessToken: ACCESS_TOKENS[shop] };
    }
    else {
        if (config && config.dev) {
            let needle = config.dev.host ? config.dev.host : 'localhost';
            if (shop == needle) {
...
const handleCallback = async (url, request, response, config) => {
    ...
    try {
        ...
        await saveShopToMemory(session, config.callbacks?.shop?.save);
        const webhookResponse = setupDeleteWebhook(params.shop, session);
        response.writeHead(302, { location: getFinalUrl(config, params), 'Shopify-Token': session.accessToken });
        response.end();
        return false;
    }
    catch (e) {
...
const handleDelete = async (url, request, response, config) => {
    await dist$1.Shopify.Utils.deleteCurrentSession(request, response);
    const shopName = url.searchParams.get('shop');
    if (shopName) {
        return await removeShopFromMemory(shopName, config.callbacks?.shop?.delete);
    }
    else {
        return false;
    }
        ...
        if (req.url) {
            switch (url.pathname) {
                ...
                case pathsToWatch.token:
                    const resp = await provideToken(url.hostname, config);
                    if (resp) {
                ...
                case pathsToWatch.callback:
                    shouldContinue = await handleCallback(url, req, res, config);
                    break;
                case pathsToWatch.delete:
                    shouldContinue = await handleDelete(url, req, res, config);
                    break;
            }
        }
        if (shouldContinue) {
...
//# sourceMappingURL=middleware-81b59bf0.js.map
