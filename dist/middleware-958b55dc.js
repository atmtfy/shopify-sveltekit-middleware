...
async function findShopInMemory(shopName, callback) {
    const resp = await tryCallback(callback, shopName);
    if (!resp) {
        return ACTIVE_SHOPIFY_SHOPS[shopName] ? ACTIVE_SHOPIFY_SHOPS[shopName] : false;
    }
    return resp;
}

let tokens = {};
async function provideToken(key, options) {
    if (key in tokens) {
        return tokens[key];
    }
    else {
        try {
            const session = await findShopInMemory(key, findHook(options, 'shop.load', key));
            if (session) {
...
const handleDelete = async (url, request, response, options) => {
    ...
    if (shopName) {
        return await removeShopFromMemory(shopName, findHook(options, "shop.delete"));
    }
    else {
        return false;
    }
...
function configureSessionStorage(options) {
    if (typeof options.sessionStorage == 'object') {
        ...
        return generateSessionStorageFromPath();
    }
}
...
async function loadConfig(options = null) {
    ...
    console.log("Trying to print config_file");
    ...
    if (options == null || typeof options != 'object') {
        ...
        if (!fs__default["default"].existsSync('shopify.config.js')) {
            ...
            console.log(url__namespace.pathToFileURL(config_file).href);
...
function checkEnv(env) {
    ...
    const envValues = { ...{
            embedded: true,
            apiVersion: dist$1.ApiVersion[0]
        }, ...env };
    let hasError = false;
        ...
        if (req.url && Object.values(pathsToWatch).includes(url.pathname)) {
            switch (url.pathname) {
                ...
                case pathsToWatch.delete:
                    ...
                    break;
                default:
                    console.log("Getting token for " + url.pathname + " if we can!");
                    const token = await provideToken(url.pathname, options);
                    if (token) {
                        res.setHeader('Shopify-Middlware-Token', token);
                    }
                    break;
            }
        }
...
//# sourceMappingURL=middleware-958b55dc.js.map
